...
const supabase = createClient(
  ...
  process.env.SUPABASE_SERVICE_ROLE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJiss3MiOiJzdXBhYmFzZSIsInJlZiI6InB4dnRmeXd1bWVrcGR0YWJsY2pxIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1NDMzOTYwNiwiZXhwIjoyMDY5OTE1NjA2fQ.N_nUTBI89ydKXQ2OWhrxz-AvnqNqjF35i_CPHUIC790'
);

// Production bypass flag - force bypass when database fails
...
class DatabaseStorage {
  ...
  async getAllUsers(): Promise<User[]> {
    // Production bypass for failing database connections
  ...
  async getSupplierPayments(supplierId?: number): Promise<any[]> {
    try {
      ...
      /* Original implementation for when table exists:
      let query = supabase
        .from('supplier_payments')
        .select('*')
        .order('created_at', { ascending: false });
  ...
  async getSupplierExpenditureSummary(): Promise<any[]> {
    try {
      // Get supplier expenditure summary with payment tracking
      const result = await sql(`
  ...
  async getDashboardData(): Promise<any> {
    try {
      // Get dashboard statistics from multiple tables
      const [
        transactionsStats,
        billsStats,
        inventoryStats,
...
// Add missing inventory methods to the class
DatabaseStorage.prototype.getInventoryItems = async function(limit?: number, offset?: number): Promise<any[]> {
  try {
    let query = supabase
      .from('inventory')
      .select('*')
...
DatabaseStorage.prototype.getInventoryItem = async function(id: number): Promise<any | null> {
  try {
    const { data, error } = await supabase
      .from('inventory')
      .select('*')
...
DatabaseStorage.prototype.updateInventoryItem = async function(id: number, data: any): Promise<any | null> {
  try {
...
DatabaseStorage.prototype.deleteInventoryItem = async function(id: number): Promise<boolean> {
  try {
    const { error } = await supabase
      .from('inventory')
      .delete()
...
DatabaseStorage.prototype.getLowStockItems = async function(): Promise<any[]> {
  try {
    const { data, error } = await supabase
      .from('inventory')
      .select('*')
