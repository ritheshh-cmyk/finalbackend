...
describe('Comprehensive API Endpoint Tests', () => {
  beforeAll(async () => {
    ...
    await sql('DELETE FROM users WHERE username = $1', ['testuser_comprehensive']);
  ...
  afterAll(async () => {
    await sql('DELETE FROM users WHERE username = $1', ['testuser_comprehensive']);
    if (transactionId) {
      await sql('DELETE FROM transactions WHERE id = $1', [transactionId]);
    }
    if (supplierId) {
      await sql('DELETE FROM suppliers WHERE id = $1', [supplierId]);
      await sql('DELETE FROM supplier_payments WHERE supplier_id = $1', [supplierId]);
    }
     if (expenditureId) {
      await sql('DELETE FROM expenditures WHERE id = $1', [expenditureId]);
    }
    if (billId) {
      await sql('DELETE FROM bills WHERE id = $1', [billId]);
    }
  ...
  describe('Transactions', () => {
    ...
    it('DELETE /api/transactions/:id - should delete a transaction', async () => {
      ...
      // Verify it's gone
      const getRes = await request(app).get(`/api/transactions/${transactionId}`).set('Authorization', `Bearer ${token}`);
  ...
  describe('Suppliers', () => {
    ...
    it('GET /api/suppliers/expenditure-summary - should get expenditure summary', async () => {
      // First, create an expenditure to have data for the summary
      await request(app)
  ...
  describe('SMS', () => {
    it('POST /api/sms/send - should send an SMS', async () => {
      ...
      // This will likely fail without a real API key, so we check for 500
