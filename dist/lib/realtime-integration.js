...
app.post('/api/transactions/realtime', requireAuth, async (req, res) => {
    try {
        ...
        const { customerName, mobileNumber, deviceModel, repairType, repairCost, paymentMethod, amountGiven, changeReturned, status, remarks } = req.body;
      ...
      `)
        ...
        if (allTransactions) {
            const realTimeMetrics = {
                ...
                completedCount: allTransactions.filter(tx => tx.status && tx.status.toLowerCase().includes('completed')).length,
...
//# sourceMappingURL=realtime-integration.js.map