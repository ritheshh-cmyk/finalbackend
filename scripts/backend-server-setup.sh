#!/usr/bin/env bash
# Modular, robust startup script for generic Node.js/TS backend
# Usage: bash backend-server-setup.sh [--no-build] [--skip-ngrok] [--debug] [--port 10000]
...
# --- Fix TypeScript permissions ---
if [ -f node_modules/.bin/tsc ]; then
  chmod +x node_modules/.bin/tsc
  log "Fixed TypeScript permissions."
fi

# --- Lint, Test, Audit, TypeCheck ---
npm run lint || log "⚠️ Lint check completed (may have warnings)"
npm run test || log "⚠️ Test check completed (may have warnings)"
npm audit --audit-level=high || log "⚠️ Vulnerabilities found in dependencies"
npm run typecheck || log "⚠️ TypeScript check completed (may have warnings)"
...
# - Place lib.sh in the backend directory or adjust the source path
